      ...
      <DialogContent className="sm:max-w-md max-h-[85vh] flex flex-col">
        {shareSuccess ? (
          <div className="py-8 text-center space-y-3">
            ...
            <h3 className="text-lg font-semibold">새 길이 열렸습니다!</h3>
            <p className="text-sm text-muted-foreground">
              다른 사람들이 걸어가면 보상을 받을 수 있어요.
            </p>
          </div>
        ) : (
        <>
        <DialogHeader>
          <DialogTitle>길 열기</DialogTitle>
          <DialogDescription>
            이 길을 커뮤니티에 열고, 첫 발자국을 남겨 자신감을 표현하세요.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4 overflow-y-auto flex-1 min-h-0 pr-1">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-sm font-medium">첫 발자국 수</label>
              <span className="text-xs text-muted-foreground">최대 {maxInvestment}👣</span>
            </div>
            <p className="text-xs text-muted-foreground mb-2">
              많이 남길수록 검색에서 더 잘 보이고, 후속 보상도 커집니다.
            </p>
            <Slider
              value={[investAmount]}
          ...
          <div className="p-3 rounded-lg bg-muted/50 text-sm space-y-1">
            <div className="flex justify-between">
              <span className="text-muted-foreground">길 열기 후 잔여 발자국:</span>
              <span className="font-mono">{balance - investAmount}👣</span>
            </div>
            <div className="text-xs text-muted-foreground mt-2">
              다른 사람이 이 길을 걸어가면 발자국의 일부가 보상으로 돌아옵니다.
            </div>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleShare} disabled={isSharing || investAmount <= 0}>
            {isSharing ? "처리 중..." : `${investAmount}👣 남기고 길 열기`}
          </Button>
        </DialogFooter>
        </>
        )}
